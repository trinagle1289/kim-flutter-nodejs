...
String resultPosture() {
  ...
  if (posture1 != 0 || posture2 != 0) {
    result +=
        '•  Maintain trunk stability: Turn with your feet, not your waist, to reduce trunk twisting.\n';
  }
  if (posture3 != 0 || posture4 != 0) {
    result +=
        '•  Keep objects close: Hold items close to your body to avoid overreaching.\n';
  }
  if (posture5 != 0 || posture6 != 0) {
    result +=
        '•  Plan workflow: Reorganize paths and processes to minimize arm lifting.\n';
  }
  if (posture7 != 0 || posture8 != 0) {
    result +=
        '•  Use tools: Operate at lower positions to reduce overhead arm movements.\n';
  }
...
String conditionresult() {
  ...
  if (isListTwoSelected && (selectedTwoValue == 1 || selectedTwoValue == 2)) {
    condi +=
        '•  Use anti-slip gloves, install assistive tools, and adjust your working posture and height.\n';
  }
  if (isListThreeSelected && selectedThreeValue == 1) {
    condi +=
        '•  For hot conditions, use air conditioners or fans; for cold or damp conditions, use insulation or waterproofing.\n';
  }
  if (isListFourSelected &&
      (selectedFourValue == 1 || selectedFourValue == 2)) {
    condi +=
        '•  Adjust the work process or rearrange the work area, and carry out site cleaning and maintenance.\n';
  }
              ...
              onTap: () {
                if (_twochecked == 0) {
                  ...
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                        builder: (context) => VideoPage(filePath: VideoPath!)),
                  );
                }
                ;
                _twochecked = -1;
